...
function SettingsDialog({ settings, onChange, onClose }: { settings: AppSettings; onChange: (settings: AppSettings) => void; onClose: () => void }) {
  ...
  const providerName = backup?.provider === 'onedrive' ? 'OneDrive folder' : backup?.provider === 'google-drive' ? 'Google Drive folder' : backup?.provider === 'local' ? 'Local folder' : 'Not configured';
  const backupReady = Boolean(backup?.folder && backup.encryptionConfigured);
  ...
  return <div className="modal-backdrop" onMouseDown={onClose}><section className="settings-dialog" role="dialog" aria-modal="true" onMouseDown={(e) => e.stopPropagation()}><header><div><h2>Settings</h2><p>Keep Noteleaf comfortable for the way you read and write.</p></div><button onClick={onClose}><X size={17} /></button></header>
    <div className="settings-scroll">
      ...
      <section className="backup-settings">
        <div className="settings-section-title"><div><h3>Backup &amp; recovery</h3><p>Save encrypted backups to any folder, including a locally synced Google Drive or OneDrive folder.</p></div><span className={`provider-badge ${backup?.provider || 'none'}`}>{providerName}</span></div>
        <div className={`backup-encryption${backup?.encryptionConfigured ? ' configured' : ''}`}>
          <div className="backup-encryption-heading"><span aria-hidden="true"><ShieldCheck size={18} /></span><div><strong>{backup?.encryptionConfigured ? 'End-to-end encryption is on' : 'Set a backup password'}</strong><small>Every new backup is encrypted on this computer with AES-256-GCM before it reaches the selected folder.</small></div></div>
          ...
          <p>Noteleaf never stores or sends the password. Keep it somewhere safe: a forgotten password cannot be recovered. Changing it affects future backups only.</p>
        </div>
        <div className="backup-folder active"><strong>{backup?.folder || 'No backup folder selected'}</strong><div><button disabled={backupBusy} onClick={() => void runBackupAction(async () => { const selected = await window.notes.backup.chooseFolder(); if (selected) setBackup(selected); })}>{backup?.folder ? 'Change backup folder…' : 'Choose backup folder…'}</button>{backup?.folder && <button disabled={backupBusy} onClick={() => void runBackupAction(() => window.notes.backup.openFolder())}>Open folder</button>}</div><small>Tip: choose a folder inside Google Drive or OneDrive if its desktop sync app is installed.</small></div>
        ...
        {!!backup?.backups.length && <div className="backup-history"><strong>Recent backups</strong>{backup.backups.slice(0, 5).map((item) => <div key={item.path}><span>{item.filename}</span><small>{(item.size / 1024 / 1024).toFixed(1)} MB</small></div>)}</div>}
        <p className="backup-note">Noteleaf never connects to or browses your cloud account. A desktop sync app may copy the encrypted backup file from your selected folder according to its own settings. <button className="backup-policy-link" onClick={() => void window.notes.system.openExternal('https://bharathgedela.github.io/Noteleaf/privacy.html')}>Privacy policy</button></p>
      </section>
    </div>
...
export function App() {
  ...
  const quickBackup = useCallback(async () => {
    ...
    if (!status.folder) {
      ...
      notify('Choose a backup folder first.');
      return;
    }
