...
Backups use a versioned authenticated-encryption envelope around the compressed archive. A user password is hardened with scrypt; a unique per-archive key is derived with HKDF and the payload is protected with AES-256-GCM. The password is never stored, while the derived local key is protected by the operating system credential store. SQLite's online backup API produces a consistent database snapshot while the app remains open. Restore extraction authenticates the encrypted payload, rejects absolute paths and path traversal, verifies each entry, runs SQLite `quick_check`, and confirms the required schema. Legacy V1 unencrypted backups remain restorable. A pending restore is applied during the next startup before the repository opens, with rollback copies retained until both the database and attachment replacement succeed.
